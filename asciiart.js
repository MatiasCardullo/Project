...
class AsciiArtGenerator {
    ...
    initGui() {
        ...
        gui
            .add(this.settings, "alpha", -1, 1, 0.01)
    ...
    generatePalettes() {
        ...
        this.colorPalettes[ColorPalette.Color4Bit] = [
            ...this.colorPalettes[ColorPalette.Color3Bit]
        ];
        for (let i = 1; i < 8; i += 1) {
    ...
    generateValueMap(ctx) {
        ...
        for (let cellY = 0; cellY < this.height; cellY += 1) {
            for (let cellX = 0; cellX < this.width; cellX += 1) {
                const cell = [];
                const pos = cellX * this.settings.charSamples * 4 +
                    cellY * this.settings.charSamples * rowLength;
        ...
        if (max > 0 && min != max) {
            ...
            for (const regions of this.valueMap) {
                ...
                for (let index = 0; index < normals.length; index += 1) {
                    ...
                    normals[index] =
                        (this.settings.contrast + 1) * (normals[index] - 0.5) +
                            0.5 +
    ...
    getCharColor(color) {
        if (this.settings.ColorPalette === ColorPalette.ColorFull) {
            ...
            for (const paletteColor of this.colorPalettes[this.settings.ColorPalette]) {
                const diff = Math.abs(color[0] - paletteColor[0]) +
